...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Include design-themes sources in the workspace.",
)
@click.pass_context
def main(
    ctx: click.Context,
    output: Optional[str],
    without_download: bool,
    ...
    needs_community = not dirs.community.exists()
    needs_enterprise = enterprise and not dirs.enterprise.exists()
    needs_themes = with_themes and not dirs.themes.exists()

    if needs_community or needs_enterprise or needs_themes:
        if not without_download:
            ...
            ctx.invoke(
                ...
                version=version,
                do_update=True,
                with_enterprise=enterprise,
                with_themes=with_themes,
            )
        else:
            print_warning("Odoo sources missing, please check before using the workspace.")

    paths = [str(dirs.community)]
    if enterprise:
        paths.append(str(dirs.enterprise))
    if with_themes:
        paths.append(str(dirs.themes))
    folders = [{"path": "."}]
    if include_sources:
