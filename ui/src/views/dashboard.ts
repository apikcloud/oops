...
export function bootDashboard(root: HTMLElement, source: BridgeSource): void {
  ...
  btnScan.disabled = btnChecks.disabled = btnDoc.disabled = true;
  toolbar.append(projectSelect, btnScan, btnChecks, btnDoc);
  root.insertAdjacentElement("beforebegin", toolbar);

  // ---- Helpers ----
  function showLoading(label: string) {
    root.innerHTML = "";
    root.append(el("div", { class: "db-loading" }, [
      el("div", { class: "db-spinner" }),
      el("span", {}, label + "…"),
    ]));
  }
  ...
  function showError(msg: string) {
    root.innerHTML = "";
    root.append(el("div", { class: "db-error" }, [
      el("strong", {}, "Error: "),
  ...
  function setButtons(on: boolean) {
    btnScan.disabled = btnChecks.disabled = btnDoc.disabled = !on;
  }
  ...
  function renderProjectInfo(payload: Payload) {
    ...
    const groups = (payload as Record<string, unknown>)["metrics"] as
      Array<{ label: string; values: Array<{ name: string; value: unknown }> }> | undefined;
    ...
    pairs
      .filter(([, v]) => v !== "—")
      .forEach(([k, v]) =>
        projectInfo.append(el("div", { class: "db-pi" }, [
