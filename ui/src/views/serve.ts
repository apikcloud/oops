...
import { el, fmt, badge, numCell, tableWrap, renderMetadataBar, renderStructure, methodKind } from "../dom";
...
export function viewServe(root: HTMLElement, payload: Payload, _source: Source): void {
    ...
    const modelOrigin = (contribs: ContributionEntry[]): string => {
        ...
        // Use root_origin (mro[-1]: original definer) so OCA extensions of core models
        // don't flip the badge to "third-party". Fall back to inherit_origin for older payloads.
        const mn = contribs[0]?.model_node as Record<string, unknown> | undefined;
        const rootOrigin = mn?.["root_origin"];
        const inheritOrigin = mn?.["inherit_origin"];
        const raw =
            typeof rootOrigin === "string" && rootOrigin
                ? rootOrigin
                : typeof inheritOrigin === "string" && inheritOrigin
                  ? inheritOrigin
                  : "unknown";
        // Bridge KB origin vocabulary (underscore) onto the soft badge family (hyphen).
        return raw === "third_party" ? "third-party" : raw;
    };
    ...
    function _viewIndex(): HTMLElement {
        ...
        setTimeout(() => {
            ...
            if (cData.length) {
                ...
                const pie = d3
                    ...
                    .sort(null);
                const arc = d3
                    .arc<d3.PieArcDatum<(typeof cData)[0]>>()
                    .innerRadius(r * 0.55)
                    .outerRadius(r - 5);
                svg.selectAll("path")
                    .data(pie(cData))
                ...
                cData.forEach((d) => {
                    ...
                    li.append("span")
                        ...
                        .style("background", CCOLORS[d.key] ?? "#999");
            ...
            if (lData.length) {
                const w = 300,
                    h = 220,
                    m = { top: 10, right: 10, bottom: 28, left: 52 };
                ...
                const x = d3
                    .scaleBand()
                    .domain(lData.map((d) => d.key))
                    .range([0, iW])
                    .padding(0.3);
                const y = d3
                    .scaleLinear()
                    .domain([0, d3.max(lData, (d) => d.value) ?? 0])
                    .nice()
                    .range([iH, 0]);
                g.selectAll("rect")
                    .data(lData)
                    ...
                    .attr("fill", (d) => LCOLORS[d.key] ?? "#999")
                    .attr("rx", 2);
                g.selectAll<SVGTextElement, (typeof lData)[0]>("text.bar-label")
                    .data(lData)
                    .join("text")
                ...
                lData.forEach((d) => {
                    ...
                    li.append("span")
                        .attr("class", "legend-swatch")
                        .style("background", LCOLORS[d.key] ?? "");
        ...
        const pillBtns = pillDefs.map(({ label, cls }) => {
            ...
            const btn = el("button", { class: "filter-btn" + (cls === "all" ? " active" : ""), "data-cls": cls }, [
                label,
                el("span", { class: "filter-count" }, count),
            ]);
        ...
        function renderTable() {
            ...
            const sorted = [...filtered].sort((a, b) => {
                ...
                if (key === "_locTotal") {
                    ...
                    av = a.node?.load_index ?? Infinity;
                    bv = b.node?.load_index ?? Infinity;
                } else if (key === "module") {
                    av = a.module ?? "";
                    bv = b.module ?? "";
                } else if (key === "classification") {
                    av = a.inventory?.classification ?? "";
                    bv = b.inventory?.classification ?? "";
                } else if (key === "version") {
                    av = a.inventory?.version ?? "";
                    bv = b.inventory?.version ?? "";
                } else {
                    av = "";
                    bv = "";
                }
    ...
    function _viewModels(): HTMLElement {
        ...
        const thead = el("thead", {}, [
            el(
                "tr",
                {},
                cols.map((c) => el("th", { class: (c.num ? "num " : "") + "sortable", "data-sort": c.key }, c.label)),
            ),
        ]);
        for (const th of thead.querySelectorAll<HTMLElement>("th[data-sort]")) {
            th.addEventListener("click", () => {
    ...
    function _viewModule(name: string): HTMLElement {
        ...
        if (mNode?.load_index != null)
            meta.push(`load order: #${mNode.load_index}` + (mNode.depth != null ? ` (depth ${mNode.depth})` : ""));
        ...
        if (mod.readme?.present && mod.readme.content) {
            ...
            card.append(
                el("pre", { style: "white-space:pre-wrap;max-height:300px;overflow-y:auto" }, mod.readme.content),
            );
        }
    ...
    function _viewModel(bare: string): HTMLElement {
        ...
        // For purely-extended (standard Odoo) models, name the original creator
        // (MRO root), not the first project module that extends it.
        const definingContrib = contributions.find((c) => {
            const s = c.model_node?.status;
            return s === "new" || s === "base";
        });
        const definingModule = definingContrib
            ? definingContrib.module
            : (contributions[0]?.model_node?.root_module ??
              contributions[0]?.model_node?.ancestor_module ??
              contributions[0]?.module);
        ...
        if (entry.description_inherited_from)
            subtitleParts.push("description from " + entry.description_inherited_from);
        ...
        const metricDefs: Array<[string, number]> = [
            ...(fTotal
                ? ([
                      ["fields total", fTotal],
                      ["fields added", fAdded],
                      ["fields inherited", fInher],
                  ] as Array<[string, number]>)
                : []),
            ...(mTotal
                ? ([
                      ["methods total", mTotal],
                      ["methods added", mAdded],
                      ["methods inherited", mInher],
                      ["methods overridden", mOver],
                  ] as Array<[string, number]>)
                : []),
        ];
        if (metricDefs.length) {
            ...
            metricDefs.forEach(([key, value]) => {
                grid.append(
                    el("div", { class: "metric" + (!value ? " zero" : "") }, [
                        el("div", { class: "metric-value" }, String(value)),
        ...
        pageFilterRow.append(
            el(
                "label",
                { style: "display:flex;align-items:center;gap:0.4rem;font-size:0.82rem;color:var(--text-muted)" },
        ...
        function renderResults(query: string) {
            ...
            for (const [kind, items] of Object.entries(groups)) {
                ...
                for (const item of items) {
                    ...
                    if (item.section) row.append(el("span", { class: `ft-chip`, style: "margin:0" }, item.section));
