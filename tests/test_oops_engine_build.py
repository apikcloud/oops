...
def _repo_ids(repo: Path, version: str = "17.0") -> list:
    """repo_ids a KBReader needs to see everything build_project_kb() wrote for `repo`."""
    return [local_repo_id(repo), odoo_core_repo_id(version)]
...
def _discover_addons(repo_path: Path, allowed_modules: "set[str]") -> "list[AddonInfo]":
    ...
    lookup (these fixtures aren't real git repos) — every addon is enriched
    ...
    addons = [a for a in seen.values() if a.technical_name in allowed_modules]
...
class TestBuildProjectKb:
    ...
    def test_missing_global_kb_raises(self, tmp_path):
        ...
        with pytest.raises(FileNotFoundError, match="Global KB not found"):
            mods = ["module_a"]
            build_project_kb(repo, "17.0", mods, _discover_addons(repo, set(mods)), global_kb=missing)
    ...
    def test_slug_override_in_meta(self, tmp_path):
        ...
        db_path = build_project_kb(
            repo, "17.0", mods, _discover_addons(repo, set(mods)), slug="my-slug", global_kb=global_kb
        ).data
    ...
    def test_build_project_kb_returns_result(self, tmp_path):
        ...
        mods = ["module_a"]
        result = build_project_kb(repo, "17.0", mods, _discover_addons(repo, set(mods)), global_kb=global_kb)
...
def _set_kb_generated_at(db_path: Path, repo_id: str, ts: datetime) -> None:
    """Overwrite one repo_id's repo_meta.generated_at row in a KB with a specific timestamp."""
    con = sqlite3.connect(str(db_path))
    con.execute(
        "UPDATE repo_meta SET value = ? WHERE key = 'generated_at' AND repo_id = ?",
        (ts.isoformat(), repo_id),
    )
    con.commit()
    con.close()
...
class TestStaleness:
    ...
    def test_fresh_no_installed_modules_no_global(self, tmp_path):
        ...
        _make_tp_symlinks(repo, "module_a")
        mods = ["module_a"]
        build_project_kb(repo, "17.0", mods, _discover_addons(repo, set(mods)), global_kb=global_kb)
    ...
    def test_installed_modules_newer(self, tmp_path):
        ...
        _set_kb_generated_at(project_db, local_repo_id(repo), past)
    ...
    def test_global_kb_newer(self, tmp_path, monkeypatch):
        ...
        _set_kb_generated_at(project_db, local_repo_id(repo), old_ts)
        new_ts = datetime(2025, 1, 1, tzinfo=timezone.utc)
        _set_kb_generated_at(global_kb, odoo_core_repo_id("17.0"), new_ts)

        # Point staleness check at our tmp global KB
        monkeypatch.setattr("oops_engine.build.global_kb_path", lambda _v: global_kb)
    ...
    def test_corrupted_generated_at(self, tmp_path):
        ...
        con.execute(
            "UPDATE repo_meta SET value = 'not-a-date' WHERE key = 'generated_at' AND repo_id = ?",
            (local_repo_id(repo),),
        )
        con.commit()
        con.close()
    ...
    def test_all_fresh(self, tmp_path, monkeypatch):
        ...
        old_ts = datetime(2020, 1, 1, tzinfo=timezone.utc)
        _set_kb_generated_at(project_db, local_repo_id(repo), new_ts)
        _set_kb_generated_at(global_kb, odoo_core_repo_id("17.0"), old_ts)
...
def _make_global_kb_with_views(path: Path) -> Path:
    ...
    write_kb(
        db_path=path,
        repo_id=odoo_core_repo_id("17.0"),
        odoo_version="17.0",
        sources={"odoo": "/odoo/src"},
        scan_results=[
            {
